...
async function readLitematic(buffer, opts = {}) {
  ...
  for (const region of Object.values(regions)) {
    ...
    for (const ent of rawEntities) {
      ...
      // TileX/Y/Z (the block an item_frame / painting hangs on) are stored in
      // the SAME region-relative frame as Pos — shift both by region anchor.
      const hasTile = typeof ent.TileX === 'number'
        && typeof ent.TileY === 'number' && typeof ent.TileZ === 'number';
      result.entities.push({
        ...
        tileX: hasTile ? ent.TileX + (pos.x || 0) : undefined,
        tileY: hasTile ? ent.TileY + (pos.y || 0) : undefined,
        tileZ: hasTile ? ent.TileZ + (pos.z || 0) : undefined,
        item: ent.Item,
    ...
    // Block entities (signs, chests, banners, ...) are counted but not parsed;
    // callers warn that their data is dropped by downstream conversions.
  ...
  for (const e of result.entities) {
    ...
    if (typeof e.tileX === 'number') {
      e.tileX -= minX; e.tileY -= minY; e.tileZ -= minZ;
    }
  }
