...
function materialDisplayName(key) {
  const raw = String(key || '').replace(/^minecraft:/, '').replace(/_/g, ' ');
  return raw.replace(/\b\w/g, (c) => c.toUpperCase());
}

function buildMaterialMetadata(blocks = []) {
  const counts = new Map();
  for (const block of blocks) {
    const key = String(block?.name || '').trim();
    if (!key || AIR_MATERIALS.has(key)) continue;
    counts.set(key, (counts.get(key) || 0) + 1);
  }
  return [...counts.entries()]
    .map(([key, count]) => ({
      key,
      name: materialDisplayName(key),
      count,
...
function buildRenderMetadata(payload) {
  return {
    name: payload.name,
    ...
    entityCount: 0,
    sourceEntityCount: payload.entityCount || 0,
    materials: buildMaterialMetadata(payload.blocks),
  };
}
  ...
  const run = _renderLock.then(async () => {
    ...
    try {
      ...
      return {
        ...
        meta: buildRenderMetadata(payload),
        diag: result.diag,
        added: result.added,
